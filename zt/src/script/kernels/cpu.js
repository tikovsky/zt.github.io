...
function timeMachine(cpu, cycle) {
    ...
    cpu.mrmovq      = cpu.history[cycle].mrmovq;
    cpu.popq        = cpu.history[cycle].popq;
...
CPU.prototype.updateCPI = function () {
    var lp = (this.mrmovq + this.popq) * this.use;
    var mp = this.cond * this.mispredict;
    var rp = this.ret;

    if (!this.instruction)
        lp = mp = rp = 0;
    if (!this.mrmovq && !this.popq)
        lp = 0;
    if (!this.cond)
        mp = 0;

    if (lp)
        lp /= this.instruction * (this.mrmovq + this.popq);
    if (mp)
...
CPU.prototype.fetch = function () {
    ...
    if ([Constant.I_NOP, Constant.I_HALT, Constant.I_RRMOVQ, Constant.I_IRMOVQ, Constant.I_RMMOVQ, Constant.I_MRMOVQ, Constant.I_OPQ, Constant.I_JXX, Constant.I_CALL, Constant.I_RET, Constant.I_PUSHQ, Constant.I_POPQ, Constant.I_LEAVE, Constant.I_IADDQ].indexOf(output.D_icode) == -1) {
    ...
    if ([Constant.I_RRMOVQ, Constant.I_OPQ, Constant.I_IRMOVQ, Constant.I_MRMOVQ, Constant.I_RMMOVQ, Constant.I_PUSHQ, Constant.I_POPQ, Constant.I_IADDQ].indexOf(output.D_icode) != -1) {
    ...
    if ([Constant.I_IRMOVQ, Constant.I_RMMOVQ, Constant.I_MRMOVQ, Constant.I_JXX, Constant.I_CALL, Constant.I_IADDQ].indexOf(output.D_icode) != -1) {
...
CPU.prototype.decode = function () {
    ...
    if ([Constant.I_RRMOVQ, Constant.I_RMMOVQ, Constant.I_OPQ, Constant.I_PUSHQ].indexOf(input.D_icode) != -1) {
        output.E_srcA = input.D_rA;
    } else if ([Constant.I_POPQ, Constant.I_RET].indexOf(input.D_icode) != -1) {
        output.E_srcA = Constant.R_RSP;
    } else if (input.D_icode == Constant.I_LEAVE) {
        output.E_srcA = Constant.R_RBP;
    } else {
        output.E_srcA = Constant.R_NONE;
    }

    if ([Constant.I_OPQ, Constant.I_RMMOVQ, Constant.I_MRMOVQ, Constant.I_IADDQ].indexOf(input.D_icode) != -1) {
        output.E_srcB = input.D_rB;
    } else if ([Constant.I_PUSHQ, Constant.I_POPQ, Constant.I_CALL, Constant.I_RET].indexOf(input.D_icode) != -1) {
        output.E_srcB = Constant.R_RSP;
    } else if (input.D_icode == Constant.I_LEAVE) {
        output.E_srcB = Constant.R_RBP;
    } else {
        output.E_srcB = Constant.R_NONE;
    }

    if ([Constant.I_RRMOVQ, Constant.I_IRMOVQ, Constant.I_OPQ, Constant.I_IADDQ].indexOf(input.D_icode) != -1) {
        output.E_dstE = input.D_rB;
    } else if ([Constant.I_PUSHQ, Constant.I_POPQ, Constant.I_CALL, Constant.I_RET, Constant.I_LEAVE].indexOf(input.D_icode) != -1) {
        output.E_dstE = Constant.R_RSP;
    } else {
        output.E_dstE = Constant.R_NONE;
    }

    if ([Constant.I_MRMOVQ, Constant.I_POPQ].indexOf(input.D_icode) != -1) {
        output.E_dstM = input.D_rA;
    } else if (input.D_icode == Constant.I_LEAVE) {
        output.E_dstM = Constant.R_RBP;
    } else {
...
CPU.prototype.execute = function () {
    ...
    if (input.E_icode == Constant.I_MRMOVQ) {
        this.mrmovq++;
    }
    if (input.E_icode == Constant.I_POPQ) {
        this.popq++;
    }
    ...
    // ALU-A
    if ([Constant.I_RRMOVQ, Constant.I_OPQ].indexOf(input.E_icode) != -1) {
        alu.config.setInA(input.E_valA);
    } else if ([Constant.I_IRMOVQ, Constant.I_RMMOVQ, Constant.I_MRMOVQ, Constant.I_IADDQ].indexOf(input.E_icode) != -1) {
        alu.config.setInA(input.E_valC);
    } else if ([Constant.I_CALL, Constant.I_PUSHQ].indexOf(input.E_icode) != -1) {
        alu.config.setInA(-4);
    } else if ([Constant.I_RET, Constant.I_POPQ, Constant.I_LEAVE].indexOf(input.E_icode) != -1) {
        alu.config.setInA(4);
    } else {
        alu.config.setInA(0);
    }
    ...
    // ALU-B
    if ([Constant.I_RMMOVQ, Constant.I_MRMOVQ, Constant.I_OPQ, Constant.I_CALL, Constant.I_PUSHQ, Constant.I_RET, Constant.I_POPQ, Constant.I_IADDQ, Constant.I_LEAVE].indexOf(input.E_icode) != -1) {
    ...
    // ALU-fCode
    if (input.E_icode == Constant.I_OPQ) {
    ...
    // ALU-Update
    if ([Constant.I_OPQ, Constant.I_IADDQ].indexOf(input.E_icode) != -1 && [Constant.STAT_ADR, Constant.STAT_INS, Constant.STAT_HLT].indexOf(output.W_stat) == -1 && [Constant.STAT_ADR, Constant.STAT_INS, Constant.STAT_HLT].indexOf(input.W_stat) == -1) {
    ...
    if (input.E_icode == Constant.I_RRMOVQ && !output.M_Cnd) {
...
CPU.prototype.memory = function () {
    ...
    if ([Constant.I_RMMOVQ, Constant.I_PUSHQ, Constant.I_CALL, Constant.I_MRMOVQ].indexOf(input.M_icode) != -1) {
        mAddr = input.M_valE;
    } else if ([Constant.I_POPQ, Constant.I_RET, Constant.I_LEAVE].indexOf(input.M_icode) != -1) {
        mAddr = input.M_valA;
    }

    rMem = [Constant.I_MRMOVQ, Constant.I_POPQ, Constant.I_RET, Constant.I_LEAVE].indexOf(input.M_icode) != -1;

    wMem = [Constant.I_RMMOVQ, Constant.I_PUSHQ, Constant.I_CALL].indexOf(input.M_icode) != -1;
...
CPU.prototype.writeBack = function () {
    ...
    if (input.W_icode == Constant.I_RMMOVQ) {
        return;
    }
...
CPU.prototype.nextPIPERegister = function () {
    ...
    var F_stall = (([Constant.I_MRMOVQ, Constant.I_POPQ, Constant.I_LEAVE].indexOf(input.E_icode) != -1) && ([output.E_srcA, output.E_srcB].indexOf(input.E_dstM) != -1)) || ([input.D_icode, input.E_icode, input.M_icode].indexOf(Constant.I_RET) != -1);

    var D_stall = ([Constant.I_MRMOVQ, Constant.I_POPQ, Constant.I_LEAVE].indexOf(input.E_icode) != -1) && ([output.E_srcA, output.E_srcB].indexOf(input.E_dstM) != -1);
    ...
    var E_bubble = (input.E_icode == Constant.I_JXX && !output.M_Cnd) || (([Constant.I_MRMOVQ, Constant.I_POPQ, Constant.I_LEAVE].indexOf(input.E_icode) != -1 && [output.E_srcA, output.E_srcB].indexOf(input.E_dstM) != -1));
    ...
    if (([Constant.I_MRMOVQ, Constant.I_POPQ].indexOf(input.E_icode) != -1) && (input.E_dstM == input.E_srcA || input.E_dstM == input.E_srcB)) {
        this.use++;
    }
